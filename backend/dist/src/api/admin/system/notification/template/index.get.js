...
exports.default = async (data) => {
    const { query } = data;
    // Support fetching all templates without pagination
    if (query.all === "true") {
        const templates = await db_1.models.notificationTemplate.findAll({
            order: [["name", "ASC"]],
            attributes: ["id", "name", "subject", "email", "sms", "push"],
        });
        return templates;
    }
