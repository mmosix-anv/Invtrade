...
exports.default = async (data) => {
    ...
    try {
        ...
        const transaction = await db_1.sequelize.transaction(async (t) => {
            ...
            ctx === null || ctx === void 0 ? void 0 : ctx.step("Creating withdrawal transaction record");
            ...
            console_1.logger.info("FOREX_WITHDRAWAL", `User ${user.id} withdrew ${amount} ${currency} from forex account ${account.id}. Transaction ID: ${transaction.id}, Wallet Type: ${type}, Chain: ${chain || 'N/A'}`);
            return transaction;
        });
        ctx === null || ctx === void 0 ? void 0 : ctx.success(`Withdrew ${amount} ${currency} from forex account ${id}${taxAmount > 0 ? ` (fee: ${taxAmount})` : ''}`);
        return {
